...
Spec: docs/shared/design-document.md#escrow
      https://bleu.github.io/byos-docs/design-document#escrow
...
This ADR settles three coupled sub-decisions from the early economics design exploration:
...
See the specification for the full withdrawal and freeze semantics, including the `_update` transfer restriction table. The key design choices:

- **All-or-nothing** eliminates partial withdrawal edge cases and balance fragmentation. A sub-solver who wants to reduce their position does a full cycle (withdraw then re-deposit).
- **Cooldown** prevents the withdraw-after-known-revert race.
...
See the specification for the full interface.
...
- **Effective balance = 0 when frozen (nuclear freeze):** Zeroing effective balance on-chain during freeze. Rejected — freeze is a withdrawal gate only; the BYOS service applies reserve logic (5x multiplier) off-chain, allowing well-capitalized sub-solvers to keep operating during investigations.
- **On-chain reserve multiplier:** Storing the 5x reserve factor in the contract. Rejected — the reserve calculation is a service-level concern, not a contract concern. Keeping it off-chain allows tuning without contract changes.
...
- **BYOS absorbs tail FX risk.** If a Track B claim's token appreciates >5x between trade and resolution, the escrow may not cover the full claim. Accepted as priced-in residual risk; primary defense is gatekeeping, not escrow recovery.
