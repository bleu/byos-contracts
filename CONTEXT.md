...
- **Proposal** — an EIP-712-signed message `{order_uid, sell_amount, buy_amount, interactions, valid_until, nonce, signature}` authorizing BYOS to attempt a settlement of those interactions and consenting to the associated escrow risk. Amounts are raw pre-fee quotes and `buy_amount` is a floor ([ADR-0003](docs/adr/0003-trampoline-deployment-settlement-integration.md), Revision). The Trampoline verifies the signature on-chain before executing ([ADR-0005](docs/adr/0005-trampoline-execution-authority.md)). A proposal commits to exactly one order, and a settlement carries exactly one proposal — batch proposals are out of scope ([ADR-0009](docs/adr/0009-single-order-solutions.md)).
- **Trampoline** — a contract that receives the route's `sellAmount`, executes the sub-solver's interactions, sweeps the sell token back to `GPv2Settlement` while enforcing `buyAmount` as a **floor** (balance-delta check, [ADR-0003](docs/adr/0003-trampoline-deployment-settlement-integration.md) Revision), and holds **no balance** outside a single settlement. Routes deliver buy-token output directly to the settlement. Confines sub-solver code to a fund-less context so it cannot exfiltrate buffers or plant exploitable approvals. Executes only in settlements submitted by a BYOS **submitter** (`tx.origin` gate, [ADR-0005](docs/adr/0005-trampoline-execution-authority.md)), so rival solvers cannot replay a proposal once its settlement calldata is public. One instance per sub-solver at a deterministic CREATE2 address, deployed at escrow-deposit time ([ADR-0001](docs/adr/0001-trampoline-topology.md), [ADR-0003](docs/adr/0003-trampoline-deployment-settlement-integration.md)).
- **Escrow** — a per-chain, native-token ERC20 contract holding sub-solver collateral keyed by sub-solver address ([ADR-0002](docs/adr/0002-escrow-contract.md), [ADR-0007](docs/adr/0007-erc20-escrow-token.md)). Tokens are minted 1:1 with deposited ETH and burned on withdrawal or debit; `balanceOf` is the single source of truth. Sub-solvers may `transfer` tokens to migrate collateral (e.g., key rotation). Transfers are restricted by pause state, freeze state, and pending withdrawal status. BYOS holds an exclusive **debit** function. Deploys the Trampoline factory from its constructor and acts as the submitter registry (`SUBMITTER_ROLE`) for its contract generation. The collateral-at-risk is the *only* sub-solver capital BYOS touches — trade capital flows atomically through `GPv2Settlement → Trampoline`.
- **Owner** — the secure wallet (multisig/Safe) that owns the Escrow. Receives debited funds, sets the operator, grants and revokes submitters, configures the cooldown. Ownership transfer is two-step.
- **Submitter** — an EOA the BYOS service submits settlements from. Holds the Escrow's `SUBMITTER_ROLE`; `Trampoline.execute` requires `tx.origin` to be one. This covers both the allow-listed solver EOA (direct submission) and the auxiliary accounts of CoW's `Solver7702Delegate` parallel path, since the auxiliary account — not the solver EOA — is `tx.origin` there ([ADR-0005](docs/adr/0005-trampoline-execution-authority.md)). Rotation is a role change by the Owner, not a redeploy.
...
- **Residue** — retired category ([ADR-0008](docs/adr/0008-residue-disposition.md), Revision 2026-07-22): `execute` sweeps the sell token to `GPv2Settlement` and routes deliver buy-token output directly, so surplus above the signed floor is BYOS-owned settlement slippage, returned by CoW's weekly accounting in native token; the claim functions are removed. Stray tokens landing on an instance are written off — a standing sub-solver approval can take them, and preventing that is out of scope.
