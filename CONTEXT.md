...
- **Proposal** — an EIP-712-signed message `{order_uid, sell_amount, buy_amount, interactions, valid_until, nonce, signature}` authorizing BYOS to attempt a settlement of those interactions and consenting to the associated escrow risk. The Trampoline verifies the signature on-chain before executing ([ADR-0005](docs/adr/0005-trampoline-execution-authority.md)). A proposal commits to exactly one order, and a settlement carries exactly one proposal — batch proposals are out of scope ([ADR-0009](docs/adr/0009-single-order-solutions.md)).
