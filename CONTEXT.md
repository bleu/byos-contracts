...
- **Submitter** — an EOA the BYOS service submits settlements from. Holds the Escrow's `SUBMITTER_ROLE`; `Trampoline.execute` requires `tx.origin` to be one. Rotation is a role change by the Owner, not a redeploy.
