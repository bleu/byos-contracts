...
contract EscrowTest is Test {
  ...
  function test_debit_reverts_non_operator() public {
    escrow.deposit{value: 10 ether}(subSolver);
    vm.prank(subSolver);
    vm.expectRevert(IEscrow.Escrow_OnlyOperator.selector);
  ...
  function test_transfer_ownership_second_call_overrides_first() public {
    ...
    vm.expectRevert(IEscrow.Escrow_OnlyPendingOwner.selector);
    escrow.acceptOwnership();
  ...
  function test_owner_functions_revert_non_owner() public {
    ...
    vm.expectRevert(IEscrow.Escrow_OnlyOwner.selector);
  ...
  function test_admin_functions_emit_events() public {
    ...
    emit IEscrow.OperatorUpdated(op, newOp);
    escrow.setOperator(newOp);
    ...
    vm.prank(owner);
    vm.expectEmit(true, true, false, false);
    emit IEscrow.OwnershipTransferStarted(owner, newOwner);
    escrow.transferOwnership(newOwner);

    vm.prank(newOwner);
    vm.expectEmit(true, true, false, false);
    emit IEscrow.OwnershipTransferred(owner, newOwner);
    escrow.acceptOwnership();
  }
  ...
  function test_withdraw_debits_emits_event() public {
    ...
    vm.expectEmit(true, false, false, true);
    emit IEscrow.DebitsWithdrawn(owner, 4 ether);
    escrow.withdrawDebits();
  }
}
