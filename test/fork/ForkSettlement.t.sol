...
contract ForkSettlementTest is Test {
    ...
    struct SignedProposal {
        Trampoline.Proposal data;
        Trampoline.Interaction[] route;
        bytes signature;
    }
    ...
    function setUp() public {
        ...
        Escrow escrow = new Escrow(makeAddr("owner"), makeAddr("operator"), 1 days, factory);
    ...
    function _quote(address sellToken, address buyToken, uint256 sellAmount) internal view returns (uint256) {
    ...
    function _swapRoute(address sellToken, address buyToken, uint256 sellAmount, uint256 buyAmountOut, bool unwrap)
        internal
        view
        returns (Trampoline.Interaction[] memory route)
    {
        address[] memory path = new address[](2);
        path[0] = sellToken;
        path[1] = buyToken;
        route = new Trampoline.Interaction[](unwrap ? 3 : 2);
        ...
        if (unwrap) {
            route[2] = Trampoline.Interaction({
                target: address(WETH), value: 0, callData: abi.encodeCall(IWETH.withdraw, (buyAmountOut))
            });
        }
    }

    function _signProposal(uint256 sellAmount, uint256 buyAmount, Trampoline.Interaction[] memory route)
        internal
        view
        returns (SignedProposal memory signed)
    {
        signed.data = Trampoline.Proposal({
    ...
    function _signOrder(address sellToken, address buyToken, uint256 sellAmount, uint256 buyAmount, uint32 validTo)
        internal
        view
    ...
    function _settleOrder(
        ...
        trades[0] = GPv2TradeData({
            ...
            signature: _signOrder(
                sellToken, buyToken, sellAmount, buyAmount * 99 / 100, uint32(block.timestamp + 1 hours)
            )
        });

        Trampoline.Interaction[][3] memory interactions;
        interactions[1] = new Trampoline.Interaction[](2);
        interactions[1][0] = Trampoline.Interaction({
            target: sellToken, value: 0, callData: abi.encodeCall(IERC20.transfer, (address(trampoline), sellAmount))
        });
        interactions[1][1] = Trampoline.Interaction({
            target: address(trampoline),
            value: 0,
            callData: abi.encodeCall(Trampoline.execute, (prop.data, prop.route, buyToken, prop.signature))
        });
    ...
    function test_fork_settle_erc20_order_through_trampoline() public onlyFork {
        ...
        SignedProposal memory prop = _signProposal(
            sellAmount, quotedOut, _swapRoute(address(WETH), address(USDC), sellAmount, quotedOut, false)
        );
