...
contract TrampolineTest is Test {
    ...
    function _proposal() internal view returns (Trampoline.Proposal memory) {
        return Trampoline.Proposal({
    ...
    function _sign(uint256 key, Trampoline.Proposal memory proposal, Trampoline.Interaction[] memory interactions)
        internal
        view
    ...
    function _swapRoute(uint256 buyAmountOut) internal view returns (Trampoline.Interaction[] memory route) {
        route = new Trampoline.Interaction[](2);
        route[0] = Trampoline.Interaction({
            target: address(sellToken),
            value: 0,
            callData: abi.encodeCall(IERC20.approve, (address(router), SELL_AMOUNT))
        });
        route[1] = Trampoline.Interaction({
    ...
    function test_execute_runs_route_and_settles_back_exact_buy_amount() public {
        ...
        Trampoline.Interaction[] memory route = _swapRoute(BUY_AMOUNT);
    ...
    function test_execute_reverts_when_caller_not_settlement() public {
        ...
        vm.expectRevert(Trampoline.OnlySettlement.selector);
        trampoline.execute(proposal, route, address(buyToken), signature);
    }
    ...
    function test_execute_reverts_when_proposal_expired() public {
        ...
        vm.prank(settlement);
        vm.expectRevert(Trampoline.ProposalExpired.selector);
        trampoline.execute(proposal, route, address(buyToken), signature);
    }
    ...
    function test_execute_reverts_on_signature_from_wrong_signer() public {
        ...
        vm.prank(settlement);
        vm.expectRevert(Trampoline.InvalidSignature.selector);
        trampoline.execute(proposal, route, address(buyToken), signature);
    }
    ...
    function test_execute_reverts_when_interactions_differ_from_signed() public {
        ...
        Trampoline.Interaction[] memory substituted = _swapRoute(BUY_AMOUNT + 1);
    ...
    function test_execute_reverts_when_route_produces_less_than_buy_amount() public {
        sellToken.mint(address(trampoline), SELL_AMOUNT);
        Trampoline.Interaction[] memory route = _swapRoute(BUY_AMOUNT - 1);
        Trampoline.Proposal memory proposal = _proposal();
        bytes memory signature = _sign(subSolverKey, proposal, route);
    ...
    function test_execute_leaves_surplus_in_instance_as_residue() public {
        ...
        sellToken.mint(address(trampoline), SELL_AMOUNT);
        Trampoline.Interaction[] memory route = _swapRoute(BUY_AMOUNT + surplus);
        Trampoline.Proposal memory proposal = _proposal();
        bytes memory signature = _sign(subSolverKey, proposal, route);
    ...
    function test_execute_bubbles_interaction_revert_data() public {
        ...
        Trampoline.Interaction[] memory route = new Trampoline.Interaction[](1);
        route[0] = Trampoline.Interaction({
            target: address(reverter), value: 0, callData: abi.encodeCall(Reverter.explode, ())
        });
        Trampoline.Proposal memory proposal = _proposal();
        bytes memory signature = _sign(subSolverKey, proposal, route);
    ...
    function test_route_cannot_call_another_instances_execute() public {
        ...
        Trampoline.Interaction[] memory innerRoute = new Trampoline.Interaction[](0);
        Trampoline.Interaction[] memory route = new Trampoline.Interaction[](1);
        route[0] = Trampoline.Interaction({
            target: address(other),
            value: 0,
            callData: abi.encodeCall(Trampoline.execute, (_proposal(), innerRoute, address(buyToken), ""))
        });
        Trampoline.Proposal memory proposal = _proposal();
        bytes memory signature = _sign(subSolverKey, proposal, route);
    ...
    function test_planted_approval_cannot_reach_other_instances_residue() public {
        ...
        route[1] = Trampoline.Interaction({
            target: address(sellToken),
            value: 0,
            callData: abi.encodeCall(IERC20.transfer, (makeAddr("sink"), SELL_AMOUNT))
        });
        Trampoline.Proposal memory proposal = _proposal();
